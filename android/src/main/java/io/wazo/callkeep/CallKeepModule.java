...
public class CallKeepModule {
    ...
    public void dispose() {
        if (voiceBroadcastReceiver == null || this._context == null)
            return;
    ...
    public boolean handleMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        switch (call.method) {
            ...
            case "displayIncomingCall": {
                displayIncomingCall((String) call.argument("uuid"), (String) call.argument("handle"),
                        (String) call.argument("localizedCallerName"));
                result.success(null);
            }
                break;
            case "answerIncomingCall": {
                ...
                break;
            case "startCall": {
                startCall((String) call.argument("uuid"), (String) call.argument("number"),
                        (String) call.argument("callerName"));
                result.success(null);
            }
                break;
            case "endCall": {
            ...
            case "checkPhoneAccountPermission": {
                checkPhoneAccountPermission(
                        new ConstraintsArray((ArrayList<Object>) call.argument("optionalPermissions")), result);
            }
                break;
            ...
            case "sendDTMF": {
                ...
                break;
            case "updateDisplay": {
                updateDisplay((String) call.argument("uuid"), (String) call.argument("displayName"),
                        (String) call.argument("handle"));
                result.success(null);
            }
                break;
    ...
    public void checkPhoneAccountPermission(ConstraintsArray optionalPermissions,
            @NonNull MethodChannel.Result result) {
        if (!isConnectionServiceAvailable()) {
            result.error(E_ACTIVITY_DOES_NOT_EXIST, "ConnectionService not available for this version of Android.",
                    null);
            return;
        }
        ...
        if (!this.hasPermissions()) {
            // requestPermissions(_currentActivity, allPermissions,
            // REQUEST_READ_PHONE_STATE);
            ArrayList<String> list = new ArrayList<String>();
            Collections.addAll(list, allPermissions);
            requestPermissions(list, /* successCallback */ new Callback() {
                @Override
    ...
    private void registerPhoneAccount(Context appContext) {
        ...
        int cap = PhoneAccount.CAPABILITY_CALL_PROVIDER;
        if (_settings != null && _settings.hasKey("phoneAccount")) {
            Log.d(TAG, "phoneAccount" + _settings.getString("phoneAccount"));
            if (_settings.getString("phoneAccount").toLowerCase().equals("capability_self_managed")) {
                cap = PhoneAccount.CAPABILITY_SELF_MANAGED;
            }
        }
        PhoneAccount.Builder builder = new PhoneAccount.Builder(handle, appName).setCapabilities(
                Build.VERSION.SDK_INT < Build.VERSION_CODES.O ? PhoneAccount.CAPABILITY_CALL_PROVIDER : cap);
    ...
    private void requestPermissions(final ArrayList<String> permissions, final Callback successCallback,
        ...
        if (activity != null) {
            PermissionUtils.requestPermissions(activity, permissions.toArray(new String[permissions.size()]), callback);
        }
    }
